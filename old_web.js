  ...
  pool.getConnection(function(err,connection){
    ...
    if(error){
      ...
      if(error2){
        ...
        runPython.runPython();
        setTimeout(function () { 
           console.log("time out waiting frequency script");
            connection.query("INSERT INTO Skill (Skill_name) SELECT distinct word_frequency.skill_name FROM word_frequency",function(error3,rows3){
              if(error3){
                ...
                connection.query('INSERT INTO job_skills (Skill_id4, Job_id4) SELECT Skill.Skill_id , Job.Job_id FROM Skill,Job,word_frequency WHERE (word_frequency.job_title = Job.Job_name) AND Skill.Skill_name = word_frequency.skill_name',function(error5,row5){
                if(error5){
                ...
                connection.query(" Select Skill.Skill_id,Skill.Skill_name,Vacancy.Vacancy_id,Vacancy.Job_id2 , Vacancy.job_description from Vacancy,Skill where Skill_name in (SELECT Skill.Skill_name FROM Skill ,job_skills,Vacancy WHERE job_skills.Job_id4 = Vacancy.Job_id2) And Vacancy.job_description Like CONCAT('% ',Skill.Skill_name,' %')",function(error6,rows6){
                if(error6){
           ...
           },180000); // python script (new_frequency.py) to finish
      }
      });
    }
    });
...
}, 60000); // waiting wuzzuf table to be ready (from csv)
...
function select_city(req,res) {
  ...
  pool.getConnection(function(err,connection){
    ...
    res.json({"code" : 100, "status" : "Error in connection database"});
      ...
      connection.on('error', function(err) {      
            res.json({"code" : 100, "status" : "Error in select_city connection database"});
            return;     
      });
});
}
